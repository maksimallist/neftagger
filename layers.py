...
def one_step_csoftmax(ten, u, mask, temp):
    """
...
def default_csoftmax(tensor, cumulative_att, t):
...
def misha_attention_block(hidden_states, window_size, sketch_dim, dim_hlayer, batch_size,
    ...
    def sketch_step(tensor, cum_attention, temper):
        ...
        before_att = tf.transpose(tensor, [1, 0, 2])  # [L; batch_size; (sketch_dim+state)*(2*window+1)]
        before_att = tf.layers.conv1d(before_att, dim_hlayer, window_size * 2 + 1, padding='same')
    ...
    for i in range(sketches_num):
        ...
        sketch_, cum_att_ = sketch_step([hidden_states, sketch], cum_att, temperature)
