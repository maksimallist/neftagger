...
train_flag['prediction_path'] = './predictions/files/'
...
def train(generator, param, flags):
    ...
    dev_data = read_dataset(join(flags['data_dir'], 'russian_test.txt'),
                            param['maximum_L'], split=False)
...
def test(generator, param, flags, checkpoint):
    ...
    test_data = read_dataset(join(flags['data_dir'], 'russian_dev.txt'),
                             param['maximum_L'], split=False)
